	...
	ata_for_each_link(link, ap, EDGE) {
		...
		if(syno_is_hw_version(HW_DS1812p) ||
			...
			if(ap && (ap->print_id == 9 || ap->print_id == 10)) {
				sata_down_spd_limit(link, 0x1);
			}
		} else if(syno_is_hw_version(HW_DS2413p) ||
				syno_is_hw_version(HW_RS2414p) ||
				syno_is_hw_version(HW_RS2414rpp)) {
		} else {
			...
			if(pdev &&
				((pdev->vendor == 0x1095 && pdev->device == 0x3132) ||
