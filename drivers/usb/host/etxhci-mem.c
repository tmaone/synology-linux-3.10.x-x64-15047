		...
		struct xhci_ring *ring, struct radix_tree_root *root)
{
	...
	addr = first_seg->dma;
	mapped_ring = radix_tree_lookup(ring->trb_address_map, addr >> TRB_SEGMENT_SHIFT);
	if (mapped_ring != ring) {
		ret = radix_tree_insert(root, addr >> TRB_SEGMENT_SHIFT, ring);
		if (ret < 0)
			goto error;
	}
		...
		struct xhci_ring *ring, struct radix_tree_root *root)
{
	...
	addr = first_seg->dma;
	radix_tree_delete(root, addr >> TRB_SEGMENT_SHIFT);
	for (seg = first_seg->next; seg != first_seg; seg = seg->next) {
		addr = seg->dma;
		radix_tree_delete(root, addr >> TRB_SEGMENT_SHIFT);
	}
...
#define XHCI_QUIRK_DONOT_DATA_TOGGLE_CLEANUP	(1 << 4)
	...
	static const struct usb_device_id quirk_device_ids[] = {
		...
		{ USB_DEVICE(0x07ca, 0x850a), .driver_info = XHCI_QUIRK_BULK_OUT_ENDPOINT_DELAY },
		{ USB_DEVICE(0x15a4, 0x9016), .driver_info = XHCI_QUIRK_BULK_OUT_ENDPOINT_DELAY },
	...
	for (i = 0; i < ARRAY_SIZE(quirk_device_ids); i++) {
		...
		if ((id->idVendor == le16_to_cpu(udev->descriptor.idVendor)) &&
			...
			if (id->driver_info & XHCI_QUIRK_BULK_OUT_ENDPOINT_DELAY) {
				virt_dev->bulk_out_udelay = 100;
			}

			if (id->driver_info & XHCI_QUIRK_MANUAL_HALT_CLEANUP) {
				virt_dev->manual_halt_cleanup = 1;
			}
