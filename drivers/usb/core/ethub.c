...
static int hub_port_disable(struct usb_hub *hub, int port1, int set_state)
{
	...
	if (hub->ports[port1 - 1]->child && set_state)
		ethub_usb_set_device_state(hub->ports[port1 - 1]->child,
...
int ethub_usb_hub_claim_port(struct usb_device *hdev, unsigned port1,
...
int ethub_usb_hub_release_port(struct usb_device *hdev, unsigned port1,
			 struct dev_state *owner)
{
	int rc;
	struct dev_state **powner;
...
void ethub_usb_hub_release_all_ports(struct usb_device *hdev, struct dev_state *owner)
{
	struct usb_hub *hub = usb_hub_to_struct_hub(hdev);
	int n;
...
#define HUB_SPEED_MORPH_TRIES	5
...
#endif  
		ethub_usb_disconnect(&hub->ports[port1 - 1]->child);
	}
	clear_bit(port1, hub->change_bits);
